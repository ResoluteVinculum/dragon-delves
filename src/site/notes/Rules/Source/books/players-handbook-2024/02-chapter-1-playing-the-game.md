---
{"dg-publish":true,"permalink":"/rules/source/books/players-handbook-2024/02-chapter-1-playing-the-game/","tags":["ttrpg-cli/compendium/src/5e/xphb"],"dg-note-properties":{"obsidianUIMode":"preview","cssclasses":["json5e-note"],"tags":["ttrpg-cli/compendium/src/5e/xphb"],"aliases":["Chapter 1: Playing the Game"]}}
---
...
The 20-sided die (`d20`) is the most important die you'll use in the game. It's central to the core mechanic—called [[Rules/Source/variant-rules/d20-test-xphb\|D20 Tests]]—the game uses to determine whether creatures succeed or fail at the things they attempt during the game (see "D20 Tests" later in this chapter). You roll a `d20` whenever your character tries to do something that the DM decides has a chance of both success and failure. The higher your roll, the more likely it is that you succeed.

#### Damage

The most common use for dice other than the `d20` is to determine damage. When you make a successful attack roll (one kind of [[Rules/Source/variant-rules/d20-test-xphb\|D20 Test]]), you roll damage to see how effective the attack is. When you cast a spell, you might also roll damage. Different weapons and spells use different dice for damage. For example, a [[Rules/Source/items/dagger-xphb\|Dagger]] uses `1d4`, a [[Rules/Source/items/greataxe-xphb\|Greataxe]] uses `1d12`, and a [[Rules/Source/spells/fireball-xphb\|Fireball]] spell uses `8d6`.
...
> 
> Part of the fun of D&D is interpreting what die rolls mean in the story. The randomness that dice bring often introduces the unexpected: an expert fails, a novice succeeds, or any number of other twists. When a [[Rules/Source/variant-rules/d20-test-xphb\|D20 Test]] or another roll produces a surprise, the Dungeon Master often gives a fun description of what happened. If you're a player, look for opportunities to interpret your character's die rolls and describe why things unfolded unexpectedly or even humorously.{ #interpreting-die-rolls-in-the-story}
...
When the outcome of an action is uncertain, the game uses a `d20` roll to determine success or failure. These rolls are called [[Rules/Source/variant-rules/d20-test-xphb\|D20 Tests]], and they come in three kinds: ability checks, saving throws, and attack rolls. They follow these steps:

- **Roll 1d20.** You always want to roll high. If the roll has [[Rules/Source/variant-rules/advantage-xphb\|Advantage]] or [[Rules/Source/variant-rules/disadvantage-xphb\|Disadvantage]] (described later in this chapter), you roll two d20s, but you use the number from only one of them—the higher one if you have Advantage or the lower one if you have Disadvantage.  
- **Add Modifiers.** Add these modifiers to the number rolled on the `d20`:  
-     - **The Relevant Ability Modifier.** This chapter and the rules glossary explain which ability modifiers to use for various [[Rules/Source/variant-rules/d20-test-xphb\|D20 Tests]].    
        - **Your Proficiency Bonus If Relevant.** Each creature has a Proficiency Bonus, a number added when making a [[Rules/Source/variant-rules/d20-test-xphb\|D20 Test]] that uses something, such as a skill, in which the creature has proficiency. See "Proficiency" later in this chapter.    
...
Add your Proficiency Bonus to an ability check when the DM determines that a skill or tool proficiency is relevant to the check and you have that proficiency. For example, if a rule refers to a Strength ([[Rules/Source/skills#Acrobatics\|Acrobatics]] or [[Rules/Source/skills#Athletics\|Athletics]]) check, you can add your Proficiency Bonus to the check if you have proficiency in the [[Rules/Source/skills#Acrobatics\|Acrobatics]] or [[Rules/Source/skills#Athletics\|Athletics]] skill. See "Proficiency" later in this chapter for more information about skill and tool proficiencies.
...
For example, if you have [[Rules/Source/variant-rules/heroic-inspiration-xphb\|Heroic Inspiration]] (see the sidebar) and roll a 3 and an 18 on an ability check that has Advantage or Disadvantage, you could expend your Heroic Inspiration to reroll one of those dice, not both of them.
...
Your Proficiency Bonus can't be added to a die roll or another number more than once. For example, if a rule allows you to make a Charisma ([[Rules/Source/skills#Deception\|Deception]] or [[Rules/Source/skills#Persuasion\|Persuasion]]) check, you add your Proficiency Bonus if you're proficient in either skill, but you don't add it twice if you're proficient in both skills.

Occasionally, a Proficiency Bonus might be multiplied or divided (doubled or halved, for example) before being added. For example, the [[Rules/Source/variant-rules/expertise-xphb\|Expertise]] feature doubles the Proficiency Bonus for certain ability checks. Whenever the bonus is used, it can be multiplied only once and divided only once.
...
If a creature is proficient in a skill, the creature applies its Proficiency Bonus to ability checks involving that skill. Without proficiency in a skill, a creature can still make ability checks involving that skill but doesn't add its Proficiency Bonus. For example, if a character tries to climb a cliff, the DM might ask for a Strength ([[Rules/Source/skills#Athletics\|Athletics]]) check. If the character has [[Rules/Source/skills#Athletics\|Athletics]] proficiency, the character adds their Proficiency Bonus to the Strength check. If the character lacks that proficiency, they make the check without adding their Proficiency Bonus.
...
> 
> Each skill proficiency is associated with an ability check. For example, the [[Rules/Source/skills#Intimidation\|Intimidation]] skill is associated with Charisma. In some situations, the DM might allow you to apply your skill proficiency to a different ability check. For example, if a character tries to intimidate someone through a show of physical strength, the DM might ask for a Strength ([[Rules/Source/skills#Intimidation\|Intimidation]]) check rather than a Charisma ([[Rules/Source/skills#Intimidation\|Intimidation]]) check. That character would make a Strength check and add their Proficiency Bonus if they have [[Rules/Source/skills#Intimidation\|Intimidation]] proficiency.{ #skills-with-different-abilities}
...
Actions can come up in other situations, too: in a social interaction, you can try to [[Rules/Source/actions#Influence\|Influence]] a creature or use the [[Rules/Source/actions#Search\|Search]] action to read the creature's body language, but you can't do both at the same time. And when you're exploring a dungeon, you can't simultaneously use the [[Rules/Source/actions#Search\|Search]] action to look for traps and use the [[Rules/Source/actions#Help\|Help]] action to aid another character who's trying to open a stuck door (with the [[Rules/Source/actions#Utilize\|Utilize]] action).
...
Certain special abilities, spells, and situations allow you to take a special action called a Reaction. A Reaction is an instant response to a trigger of some kind, which can occur on your turn or on someone else's. The [[Rules/Source/actions#Opportunity Attack\|Opportunity Attack]], described later in this chapter, is the most common type of Reaction.
...
An NPC's attitude toward your character is [[Rules/Source/variant-rules/friendly-attitude-xphb\|Friendly]], [[Rules/Source/variant-rules/indifferent-attitude-xphb\|Indifferent]], or [[Rules/Source/variant-rules/hostile-attitude-xphb\|Hostile]], as defined in the rules glossary. Friendly NPCs are predisposed to help, and Hostile ones are inclined to hinder.
...
Ability checks can be key in determining the outcome of a social interaction. Your roleplaying efforts can alter an NPC's attitude, but there might still be an element of chance if the DM wants dice to play a role in determining an NPC's response to you. In such situations, the DM will typically ask you to take the [[Rules/Source/actions#Influence\|Influence]] action.

Pay attention to your skill proficiencies when thinking of how you will interact with an NPC; use an approach that relies on your group's skill proficiencies. For example, if the group needs to trick a guard into letting them into a castle, the Rogue who is proficient in [[Rules/Source/skills#Deception\|Deception]] should lead the discussion.

### Social Interaction Example

![/Rules/Source/books/players-handbook-2024/img/005-01-006-social-interaction-example.webp\|center wmed](/img/user/Rules/Source/books/players-handbook-2024/img/005-01-006-social-interaction-example.webp)
...
> 
> Phillip's character, Gareth, makes a Wisdom ([[Rules/Source/skills#Insight\|Insight]]) check to determine whether he reads Ismark's unspoken cues. Wisdom is the ability that measures perceptiveness and intuition. Insight reflects a character's skill at reading other people's moods and intentions. The DM set the DC at 15, which Gareth beats. With a successful Wisdom ([[Rules/Source/skills#Insight\|Insight]]) check, Gareth gleans information that wasn't obvious, so it wasn't part of the DM's earlier narration. For more information, see "Ability Scores" and "Proficiency" earlier in this chapter and the [[Rules/Source/actions#Search\|Search]] action in the rules glossary.{ #1-ability-checks}
...
> 
> Gareth takes the [[Rules/Source/actions#Influence\|Influence]] action, which characters can take to influence an NPC. It's easier to influence an NPC who's already disposed to help you, as Ismark is here. Ismark's [[Rules/Source/variant-rules/friendly-attitude-xphb\|Friendly]] attitude gives Phillip Advantage on the roll. In this case, [[Rules/Source/variant-rules/advantage-xphb\|Advantage]] made the difference between success and failure.{ #2-influencing-npcs}
...
As adventurers explore, their equipment can help them in many ways. For example, they can reach out-of-the-way places with a Ladder, perceive things they wouldn't otherwise notice with a Torch or another light source, bypass locked doors and containers with [[Rules/Source/items/thieves-tools-xphb\|Thieves' Tools]], and create obstacles for pursuers with [[Rules/Source/items/caltrops-xphb\|Caltrops]].

See chapter 6 for rules on many items that are useful on adventures. The items in that chapter's "Tools" and "Adventuring Gear" sections are especially useful. The weapons in that chapter can also be used for more than battle; you could use a [[Rules/Source/items/quarterstaff-xphb\|Quarterstaff]], for example, to push a sinister-looking button that you're reluctant to touch.

### Vision and Light

![/Rules/Source/books/players-handbook-2024/img/006-01-007-obscured-exploration.webp\|center wmed](/img/user/Rules/Source/books/players-handbook-2024/img/006-01-007-obscured-exploration.webp)
...
An area might be Lightly or Heavily Obscured. In a Lightly Obscured area—such as an area with Dim Light, patchy fog, or moderate foliage—you have [[Rules/Source/variant-rules/disadvantage-xphb\|Disadvantage]] on Wisdom ([[Rules/Source/skills#Perception\|Perception]]) checks that rely on sight.

A Heavily Obscured area—such as an area with Darkness, heavy fog, or dense foliage—is opaque. You have the [[Rules/Source/conditions#Blinded\|Blinded]] condition when trying to see something there.
...
Dim Light, also called shadows, creates a [[Rules/Source/variant-rules/lightly-obscured-xphb\|Lightly Obscured]] area. An area of Dim Light is usually a boundary between Bright Light and surrounding Darkness. The soft light of twilight and dawn also counts as Dim Light. A full moon might bathe the land in Dim Light.

##### Darkness

Darkness creates a [[Rules/Source/variant-rules/heavily-obscured-xphb\|Heavily Obscured]] area. Characters face Darkness outdoors at night (even most moonlit nights), within the confines of an unlit dungeon, or in an area of magical Darkness.
...
[[Rules/Source/senses#Blindsight\|Blindsight]]

[[Rules/Source/senses#Darkvision\|Darkvision]]

[[Rules/Source/senses#Tremorsense\|Tremorsense]]

[[Rules/Source/senses#Truesight\|Truesight]]

### Hiding

Adventurers and monsters often hide, whether to spy on one another, sneak past a guardian, or set an ambush. The Dungeon Master decides when circumstances are appropriate for hiding. When you try to hide, you take the [[Rules/Source/actions#Hide\|Hide]] action.
...
When time is short, such as in combat, interactions with objects are limited: one free interaction per turn. That interaction must occur during a creature's movement or action. Any additional interactions require the [[Rules/Source/actions#Utilize\|Utilize]] action, as explained in "Combat" later in this chapter.

#### Finding Hidden Objects

When your character searches for hidden things, such as a secret door or a trap, the DM typically asks you to make a Wisdom ([[Rules/Source/skills#Perception\|Perception]]) check, provided you describe the character searching in the hidden object's vicinity. On a success, you find the object, other important details, or both.

If you describe your character searching nowhere near a hidden object, a Wisdom ([[Rules/Source/skills#Perception\|Perception]]) check won't reveal the object, no matter the check's total.

#### Carrying Objects

You can usually carry your gear and treasure without worrying about the weight of those objects. If you try to haul an unusually heavy object or a massive number of lighter objects, the DM might require you to abide by the rules for [[Rules/Source/variant-rules/carrying-capacity-xphb\|carrying capacity]] in the rules glossary.

#### Breaking Objects

As an action, you can automatically break or otherwise destroy a fragile, nonmagical object, such as a glass container or a piece of paper. If you try to damage something more resilient, the DM might use the rules on [[Rules/Source/variant-rules/breaking-objects-xphb\|breaking objects]] in the rules glossary.

### Hazards

Monsters are the main perils characters face, but other dangers await. The rules glossary defines the following hazards:

[[Rules/Source/traps-hazards/burning-xphb\|Burning]]

[[Rules/Source/traps-hazards/dehydration-xphb\|Dehydration]]

[[Rules/Source/traps-hazards/falling-xphb\|Falling]]

[[Rules/Source/traps-hazards/malnutrition-xphb\|Malnutrition]]
...
Traveling at a Fast pace imposes [[Rules/Source/variant-rules/disadvantage-xphb\|Disadvantage]] on a traveler's Wisdom ([[Rules/Source/skills#Perception\|Perception]] or [[Rules/Source/skills#Survival\|Survival]]) and Dexterity ([[Rules/Source/skills#Stealth\|Stealth]]) checks.

##### Normal

Traveling at a Normal pace imposes [[Rules/Source/variant-rules/disadvantage-xphb\|Disadvantage]] on Dexterity ([[Rules/Source/skills#Stealth\|Stealth]]) checks.

##### Slow

Traveling at a Slow pace grants [[Rules/Source/variant-rules/advantage-xphb\|Advantage]] on Wisdom ([[Rules/Source/skills#Perception\|Perception]] or [[Rules/Source/skills#Survival\|Survival]]) checks.
...
![/Rules/Source/books/players-handbook-2024/img/007-01-008-exploration-example.webp\|center wmed](/img/user/Rules/Source/books/players-handbook-2024/img/007-01-008-exploration-example.webp)
...
> 
> Adventure environments are full of secrets: traps, concealed doors, loose floorboards, and hidden treasures. Characters often use Wisdom checks to find things—see the [[Rules/Source/actions#Search\|Search]] action in the rules glossary—but sometimes they stumble on them accidentally, as Auro did here.{ #4-finding-hidden-objects}
...
> 
> A saving throw is used to see whether a creature avoids or resists something. When the chest releases toxic gas, Phillip's and Amy's characters make Constitution saving throws. Constitution is one of the six abilities that measure creatures' capabilities, and it has to do with health and stamina. When they fail their saves, the gas causes them to have the [[Rules/Source/conditions#Paralyzed\|Paralyzed]] condition. See "Conditions" later in this chapter for more information.{ #7-saving-throws}



## Combat

![/Rules/Source/books/players-handbook-2024/img/008-01-009-adventurers-in-combat.webp\|center wmed](/img/user/Rules/Source/books/players-handbook-2024/img/008-01-009-adventurers-in-combat.webp)

Adventurers encounter many dangerous monsters and nefarious villains. In those moments, combat often breaks out.

### The Order of Combat

A typical combat encounter is a clash between two sides: a flurry of weapon swings, feints, parries, footwork, and spellcasting. The game organizes combat into a cycle of rounds and turns. A round represents about 6 seconds in the game world. During a round, each participant in a battle takes a turn. The order of turns is determined at the beginning of combat when everyone rolls [[Rules/Source/variant-rules/initiative-xphb\|Initiative]]. Once everyone has taken a turn, the fight continues to the next round if neither side is defeated.
...
- **Roll Initiative.** Everyone involved in the combat encounter rolls [[Rules/Source/variant-rules/initiative-xphb\|Initiative]], determining the order of combatants' turns.  
...
If a combatant is surprised by combat starting, that combatant has [[Rules/Source/variant-rules/disadvantage-xphb\|Disadvantage]] on their Initiative roll. For example, if an ambusher starts combat while hidden from a foe who is unaware that combat is starting, that foe is surprised.
...
On your turn, you can move a distance up to your [[Rules/Source/variant-rules/speed-xphb\|Speed]] and take one action. You decide whether to move first or take your action first.
...
If you want to interact with a second object, you need to take the [[Rules/Source/actions#Utilize\|Utilize]] action. Some magic items and other special objects always require an action to use, as stated in their descriptions.

The DM might require you to use an action for any of these activities when it needs special care or when it presents an unusual obstacle. For instance, the DM might require you to take the [[Rules/Source/actions#Utilize\|Utilize]] action to open a stuck door or turn a crank to lower a drawbridge.

##### Doing Nothing on Your Turn

You can forgo moving, taking an action, or doing anything at all on your turn. If you can't decide what to do, consider taking the defensive [[Rules/Source/actions#Dodge\|Dodge]] action or the [[Rules/Source/actions#Ready\|Ready]] action to delay acting.
...
Your movement can include [[Rules/Source/variant-rules/climbing-xphb\|climbing]], [[Rules/Source/variant-rules/crawling-xphb\|crawling]], [[Rules/Source/variant-rules/jumping-xphb\|jumping]], and [[Rules/Source/variant-rules/swimming-xphb\|swimming]] (each explained in the rules glossary). These different modes of movement can be combined with your regular movement, or they can constitute your entire move.
...
A character's Speed is determined during character creation. A monster's Speed is noted in the monster's stat block. See the rules glossary for more about Speed as well as about special speeds, such as a [[Rules/Source/variant-rules/climb-speed-xphb\|Climb Speed]], [[Rules/Source/variant-rules/fly-speed-xphb\|Fly Speed]], or [[Rules/Source/variant-rules/swim-speed-xphb\|Swim Speed]].
...
On your turn, you can give yourself the [[Rules/Source/conditions#Prone\|Prone]] condition without using an action or any of your Speed, but you can't do so if your Speed is 0.
...
During your move, you can pass through the space of an ally, a creature that has the [[Rules/Source/conditions#Incapacitated\|Incapacitated]] condition, a Tiny creature, or a creature that is two sizes larger or smaller than you.

Another creature's space is [[Rules/Source/variant-rules/difficult-terrain-xphb\|Difficult Terrain]] for you unless that creature is Tiny or your ally.

You can't willingly end a move in a space occupied by another creature. If you somehow end a turn in a space with another creature, you have the [[Rules/Source/conditions#Prone\|Prone]] condition unless you are Tiny or are of a larger size than the other creature.

### Making an Attack

When you take the [[Rules/Source/actions#Attack\|Attack]] action, you make an attack. Some other actions, Bonus Actions, and Reactions also let you make an attack. Whether you strike with a Melee weapon, fire a Ranged weapon, or make an attack roll as part of a spell, an attack has the following structure:

- **Choose a Target.** Pick a target within your attack's range: a creature, an object, or a location.  
- **Determine Modifiers.** The DM determines whether the target has [[Rules/Source/variant-rules/cover-xphb\|Cover]] (see the next section) and whether you have [[Rules/Source/variant-rules/advantage-xphb\|Advantage]] or [[Rules/Source/variant-rules/disadvantage-xphb\|Disadvantage]] against the target. In addition, spells, special abilities, and other effects can apply penalties or bonuses to your attack roll.  
...
> 
> When you make an attack roll against a target you can't see, you have [[Rules/Source/variant-rules/disadvantage-xphb\|Disadvantage]] on the roll. This is true whether you're guessing the target's location or targeting a creature you can hear but not see. If the target isn't in the location you targeted, you miss.
> 
> When a creature can't see you, you have [[Rules/Source/variant-rules/advantage-xphb\|Advantage]] on attack rolls against it.
> 
...
Some ranged attacks, such as those made with a [[Rules/Source/items/longbow-xphb\|Longbow]], have two ranges. The smaller number is the normal range, and the larger number is the long range. Your attack roll has [[Rules/Source/variant-rules/disadvantage-xphb\|Disadvantage]] when your target is beyond normal range, and you can't attack a target beyond long range.

#### Ranged Attacks in Close Combat

Aiming a ranged attack is more difficult when a foe is next to you. When you make a ranged attack roll with a weapon, a spell, or some other means, you have [[Rules/Source/variant-rules/disadvantage-xphb\|Disadvantage]] on the roll if you are within 5 feet of an enemy who can see you and doesn't have the [[Rules/Source/conditions#Incapacitated\|Incapacitated]] condition.

### Melee Attacks

A melee attack allows you to attack a target within your reach. A melee attack typically uses a handheld weapon or an [[Rules/Source/variant-rules/unarmed-strike-xphb\|Unarmed Strike]]. Many monsters make melee attacks with claws, teeth, or other body parts. A few spells also involve melee attacks.
...
You can avoid provoking an Opportunity Attack by taking the [[Rules/Source/actions#Disengage\|Disengage]] action. You also don't provoke an Opportunity Attack when you [[Rules/Source/variant-rules/teleportation-xphb\|Teleport]] or when you are moved without using your movement, action, Bonus Action, or Reaction. For example, you don't provoke an Opportunity Attack if an explosion hurls you out of a foe's reach or if you fall past an enemy.

##### Making an Opportunity Attack

You can make an Opportunity Attack when a creature that you can see leaves your reach. To make the attack, take a Reaction to make one melee attack with a weapon or an [[Rules/Source/variant-rules/unarmed-strike-xphb\|Unarmed Strike]] against that creature. The attack occurs right before it leaves your reach.
...
During your move, you can mount a creature that is within 5 feet of you or dismount. Doing so costs an amount of movement equal to half your [[Rules/Source/variant-rules/speed-xphb\|Speed]] (round down). For example, if your Speed is 30 feet, you spend 15 feet of movement to mount a horse.
...
The [[Rules/Source/variant-rules/initiative-xphb\|Initiative]] of a controlled mount changes to match yours when you mount it. It moves on your turn as you direct it, and it has only three action options during that turn: [[Rules/Source/actions#Dash\|Dash]], [[Rules/Source/actions#Disengage\|Disengage]], and [[Rules/Source/actions#Dodge\|Dodge]]. A controlled mount can move and act even on the turn that you mount it.
...
If an effect is about to move your mount against its will while you're on it, you must succeed on a DC 10 Dexterity saving throw or fall off, landing with the [[Rules/Source/conditions#Prone\|Prone]] condition in an unoccupied space within 5 feet of the mount.

While mounted, you must make the same save if you're knocked [[Rules/Source/conditions#Prone\|Prone]] or the mount is.
...
When making a melee attack roll with a weapon underwater, a creature that lacks a [[Rules/Source/variant-rules/swim-speed-xphb\|Swim Speed]] has [[Rules/Source/variant-rules/disadvantage-xphb\|Disadvantage]] on the attack roll unless the weapon deals Piercing damage.
...
> [!note] Resting
> 
> Adventurers can't spend every hour adventuring. They need rest. Any creature can take hour-long [[Rules/Source/variant-rules/short-rest-xphb\|Short Rests]] in the midst of a day and an 8-hour [[Rules/Source/variant-rules/long-rest-xphb\|Long Rest]] to end it. Regaining Hit Points is one of the main benefits of a rest. The rules glossary provides the rules for Short and Long Rests.{ #resting}
...
When attacking with a weapon, you add your ability modifier—the same modifier used for the attack roll—to the damage roll. A spell tells you which dice to roll for damage and whether to add any modifiers. Unless a rule says otherwise, you don't add your ability modifier to a fixed damage amount that doesn't use a roll, such as the damage of a [[Rules/Source/items/blowgun-xphb\|Blowgun]]. See chapter 6 for weapons' damage dice and chapter 7 for spells' damage dice.

### Critical Hits

When you score a Critical Hit, you deal extra damage. Roll the attack's damage dice twice, add them together, and add any relevant modifiers as normal. For example, if you score a Critical Hit with a [[Rules/Source/items/dagger-xphb\|Dagger]], roll `2d4` for the damage rather than `1d4`, and add your relevant ability modifier. If the attack involves other damage dice, such as from the Rogue's Sneak Attack feature, you also roll those dice twice.
...
When you create a damaging effect that forces two or more targets to make saving throws against it at the same time, roll the damage once for all the targets. For example, when a wizard casts [[Rules/Source/spells/fireball-xphb\|Fireball]], the spell's damage is rolled once for all creatures caught in the blast.
...
Each instance of damage has a type, like Fire or Slashing. Damage types are listed in the rules glossary and have no rules of their own, but other rules, such as [[Rules/Source/variant-rules/resistance-xphb\|Resistance]], rely on damage types.
...
Hit Points can be restored by magic, such as the [[Rules/Source/spells/cure-wounds-xphb\|Cure Wounds]] spell or a [[Rules/Source/items/potion-of-healing-xdmg\|Potion of Healing]], or by a [[Rules/Source/variant-rules/short-rest-xphb\|Short]] or [[Rules/Source/variant-rules/long-rest-xphb\|Long Rest]].
...
When a creature drops to 0 Hit Points, it either dies outright or falls [[Rules/Source/conditions#Unconscious\|Unconscious]], as explained below.
...
If your character dies, others might find a magical way to revive your character, such as with the [[Rules/Source/spells/raise-dead-xphb\|Raise Dead]] spell. Or talk with the DM about making a new character to join the group. The rules glossary has more information on being [[Rules/Source/variant-rules/dead-xphb\|dead]].

#### Falling Unconscious

If you reach 0 Hit Points and don't die instantly, you have the [[Rules/Source/conditions#Unconscious\|Unconscious]] condition until you regain any Hit Points, and you now face making [[Rules/Source/variant-rules/death-saving-throw-xphb\|Death Saving Throws]] (see below).

> [!note] Knocking Out a Creature
> 
> When you would reduce a creature to 0 Hit Points with a melee attack, you can instead reduce the creature to 1 Hit Point and give it the [[Rules/Source/conditions#Unconscious\|Unconscious]] condition. It then starts a [[Rules/Source/variant-rules/short-rest-xphb\|Short Rest]], at the end of which that condition ends on it. The condition ends early if the creature regains any Hit Points or if someone takes an action to administer first aid to it, making a successful DC 10 Wisdom ([[Rules/Source/skills#Medicine\|Medicine]]) check.{ #knocking-out-a-creature}
...
Roll `1d20`. If the roll is 10 or higher, you succeed. Otherwise, you fail. A success or failure has no effect by itself. On your third success, you become [[Rules/Source/variant-rules/stable-xphb\|Stable]] (see "Stabilizing a Character" below). On your third failure, you die.
...
You can take the [[Rules/Source/actions#Help\|Help]] action to try to stabilize a creature with 0 Hit Points, which requires a successful DC 10 Wisdom ([[Rules/Source/skills#Medicine\|Medicine]]) check.

A Stable creature doesn't make [[Rules/Source/variant-rules/death-saving-throw-xphb\|Death Saving Throws]] even though it has 0 Hit Points, but it still has the [[Rules/Source/conditions#Unconscious\|Unconscious]] condition. If the creature takes damage, it stops being Stable and starts making [[Rules/Source/variant-rules/death-saving-throw-xphb\|Death Saving Throws]] again. A Stable creature that isn't healed regains 1 Hit Point after `1d4` hours.
...
Temporary Hit Points last until they're depleted or you finish a [[Rules/Source/variant-rules/long-rest-xphb\|Long Rest]].
...
> 
> The next step in combat is for everyone involved to roll [[Rules/Source/variant-rules/initiative-xphb\|Initiative]].{ #9-rolling-initiative}
...
> 
> When a creature attacks, whether swinging a sword or firing an arrow from a bow, its player makes an attack roll. If the total is equal to or higher than the target's [[Rules/Source/variant-rules/armor-class-xphb\|Armor Class]] (AC), the attack hits.{ #11-attack-rolls}



> [!note] (12)—Spells and Reactions
> 
> Mirabella casts the [[Rules/Source/spells/shield-xphb\|Shield]] spell. It's a special property of this spell that it can be cast even when it's not the caster's turn, using her Reaction (see "Actions" earlier in this chapter).{ #12-spells-and-reactions}



> [!note] (13)—Special Abilities
> 
> Auro's Sneak Attack lets him deal extra damage to the skeletons, and his Cunning Action lets him [[Rules/Source/actions#Disengage\|Disengage]] as a Bonus Action. These are examples of class features (described in chapter 3) that let characters bend the rules.{ #13-special-abilities}
...
> 
> Spells and many other effects in the game fill a specific area that is defined by the rules, like the 15-foot [[Rules/Source/variant-rules/cube-area-of-effect-xphb\|Cube]] of the [[Rules/Source/spells/thunderwave-xphb\|Thunderwave]] spell. See the rules glossary for more on areas of effect.{ #15-areas-of-effect}



> [!note] (16)—Saving Throws
> 
> The skeletons make Constitution saving throws to resist the [[Rules/Source/spells/thunderwave-xphb\|Thunderwave]] spell. When they fail those saves, they take damage and are pushed, as defined in that spell's description.{ #16-saving-throws}
...
> 
> Gareth's Sear Undead is a class feature (see chapter 3). As with Mirabella's [[Rules/Source/spells/thunderwave-xphb\|Thunderwave]] spell, Sear Undead forces the skeletons to make a saving throw.{ #17-saving-throws}



![/Rules/Source/books/players-handbook-2024/img/011-01-012-combat-example.webp\|center wmed](/img/user/Rules/Source/books/players-handbook-2024/img/011-01-012-combat-example.webp)